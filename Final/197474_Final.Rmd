...
$$Y_{it} = β0 + β1*[Time] + β2*[Vote Center] + β3*[Time*Vote Center] + β4*[Median Income] + β5*[Potal Population] + β6*[Bachelor Degree] + q_{it}$$
...
tab_model(tb1_p, tb1_m, tb1_c, 
          ...
          pred.labels=c("Vote center (1=yes/0=no)", "Time_P (1=2016/0=2012)", "Median income", "Total population", "Bachelor's degree or higher", "Time_P*Vote center", "Time_M (1=2018/0=2010)", "Time_M*Vote center", "Time_C (1=2017/0=2019)", "Time_C*Vote center"),
...
tab_model(tb3_p, tb3_m, tb3_c, 
          ...
          robust=TRUE,
          dv.labels = c("Presidential", "Midterm", "Constitutional"),
          pred.labels=c("Constant", "Vote center (1=yes/0=no)", "Time_P (1=2016/0=2012)", "Median income", "Total population", "Bachelor's degree or higher", "Time_P*Vote center", "Time_M (1=2018/0=2010)", "Time_M*Vote center", "Time_C (1=2017/0=2019)", "Time_C*Vote center"),
          string.pred="Variables",
...
## 8.2 Placebo test: alternative outcomes

For an additional diagnostics, we can re-code some control groups as treated and re-estimate the diff-in-diff estimand. Since the result of this paper was neither substantially nor statistically significant, the placebo test should produce similar results. 

```{r results="asis"}
# create dummy variables that assign the control group to treatment group and vice versa. 
placebo <- a %>% mutate(
              time_p = 
               case_when(
                 year == 2012 ~ "1",
...
# Presidential
placebo_p <- plm(turnout ~ votecenter*time_p + medianinc_rec + totalpop_rec + bachelor_higher, data=placebo, index=c("county"), model="within")
...
tab_model(placebo_p, placebo_m, placebo_c, 
          show.ci=FALSE, collapse.se=TRUE, p.style="asterisk", show.intercept=TRUE,
          title="Replicated Table 1",
          robust=TRUE,
          dv.labels = c("Presidential", "Midterm", "Constitutional"),
...
The regression results have produced estimands that are close to 0, which are not identical, but consistent with the original results. Therefore, the diff-in-diff results appear robust. 